...
{
    public class PrintOsobljeCommand : ICommand
    {
        public event EventHandler CanExecuteChanged;

        private KuhinjaViewModel _currentViewModel;

        public PrintOsobljeCommand(KuhinjaViewModel currentViewModel)
        {
        ...
        public void Execute(object parameter)
        {
            try
            {
                KuhinjaReport kuhinjaReport = new KuhinjaReport()
                {
                    ...
                    $"Konobari za {_currentViewModel.FromDate.ToString("dd.MM.yyyy")}" :
                     $"Konobari za {_currentViewModel.FromDate.ToString("dd.MM.yyyy")} - {_currentViewModel.ToDate.ToString("dd.MM.yyyy")}",
                    Total = 0,
                ...
                using (var dbContext = _currentViewModel.DbContext.CreateDbContext())
                {
                    var allOrdersToday = dbContext.OrdersToday.Include(o => o.OrderTodayItems)
                        ...
                        (o.PaymentPlace.Name.ToLower().Contains("konobar") || o.PaymentPlace.Name.ToLower().Contains("konobari")) &&
            ...
            {
                Log.Error("PrintOsobljeCommand -> desila se greska prilikom stampe arikala u konobara: ", ex);
